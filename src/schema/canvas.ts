...
export type CanvasIdRef = Type.Static<typeof CanvasIdRef>;

/** Nest slots always point at another canvas by id (recursion via ecosystem resolution). */
export type CanvasSlot = CanvasIdRef;
const CanvasSlot = CanvasIdRef;

/**
 * Fractal Lean Canvas TypeBox schema (9 Lean Canvas dimensions).
 * Document versioning lives on VersionedFractalEnvelope, not on nest nodes.
 * Nest slots are `{ id }` to another canvas — never inline, never file paths.
 */
export const FractalLeanCanvas = Type.Object(
  {
    id: Type.String({ minLength: 1 }),
